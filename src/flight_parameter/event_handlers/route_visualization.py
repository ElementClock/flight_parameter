...
class RouteVisualizationHandler(BaseEventHandler):
        ...
        try:
            ...
            with wx.FileDialog(
                ...
                if len(sheet_names) == 1:
                    self.draw_route_from_sheet(pathname, sheet_names[0])
                else:
                    # 如果有多个工作表，让用户选择
                    ...
                    if dialog.ShowModal() == wx.ID_OK:
                        ...
                        self.draw_route_from_sheet(pathname, selected_sheet)
                    dialog.Destroy()
    ...
    def draw_route_from_sheet(self, file_path, sheet_name):
        """
        从指定的工作表绘制航线
        ...
        try:
            ...
            if not waypoints:
                wx.MessageBox(f"未找到{sheet_name}中的航路点数据", "提示", wx.OK | wx.ICON_INFORMATION)
                return
            
            # 创建临时文件路径用于保存图表
            temp_dir = wx.StandardPaths.Get().GetTempDir()
            temp_file = os.path.join(temp_dir, f"route_map_{sheet_name}.png")
            
            # 绘制航路图
            self.draw_route_map(waypoints, route_order, f'航路图（{sheet_name}）', temp_file, capital_cities, 
                               PROVINCE_TO_CAPITAL, HIGH_CONTRAST_COLORS)
            
            # 显示图片
            if os.path.exists(temp_file):
                wx.MessageBox(f"航路图已生成并保存到: {temp_file}", "提示", wx.OK | wx.ICON_INFORMATION)
                ...
                wx.MessageBox("生成航路图失败", "错误", wx.OK | wx.ICON_ERROR)
                
        except ImportError as e:
            wx.MessageBox(f"缺少必要的绘图库: {str(e)}\n请安装所需的依赖库", "错误", wx.OK | wx.ICON_ERROR)
        except Exception as e:
            logging.error(f"绘制航路图时出错: {str(e)}")
            wx.MessageBox(f"绘制航路图时出错: {str(e)}", "错误", wx.OK | wx.ICON_ERROR)
    
    def draw_route_map(self, waypoints, route_order, title, save_path, capital_cities, 
                       PROVINCE_TO_CAPITAL, HIGH_CONTRAST_COLORS):
        """
        绘制航路图
        ...
        title (str): 图表标题
        save_path (str): 保存路径
        capital_cities (dict): 省会城市数据
        PROVINCE_TO_CAPITAL (dict): 省份到省会城市的映射
        HIGH_CONTRAST_COLORS (list): 高对比度颜色列表
        """
        import matplotlib
        matplotlib.use('Agg')  # 使用非交互式后端
        import matplotlib.pyplot as plt

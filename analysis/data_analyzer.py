...
class DataAnalyzer:
    ...
    def convert_flight_time(self, df):
        ...
        try:
            # 获取第4、5、6列的列名
            date_col, flag_col, time_col = self._get_time_column_names(df)
            
            # 过滤标识符为1的可信数据
            df_filtered = self._filter_reliable_data(df, flag_col)
            ...
            # 解析日期列
            parsed_dates = self._parse_date_column(df_filtered, date_col)
            ...
            if flag_col in df_filtered.columns:
                df_filtered.drop([flag_col], axis=1, inplace=True)
            if time_col in df_filtered.columns and time_col != '飞行时间':
                df_filtered.drop([time_col], axis=1, inplace=True)
    ...
    def _get_time_column_names(self, df):
        ...
        if len(df.columns) < 6:
            ...
            if len(time_columns) >= 3:
                date_col = time_columns[0]
                flag_col = time_columns[1] 
                time_col = time_columns[2]
            else:
        ...
        return date_col, flag_col, time_col
    
    def _filter_reliable_data(self, df, flag_col):
        """过滤标识符为1的可信数据
    ...
    def _parse_date_column(self, df_filtered, date_col):
        """解析日期列
